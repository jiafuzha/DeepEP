...
def test_main(args: argparse.Namespace,
    ...
    # Random data
    x = torch.ones((num_tokens, hidden), dtype=torch.bfloat16, device='xpu') * rank
    x_pure_rand = torch.randn((num_tokens, hidden), dtype=torch.bfloat16, device='xpu')
    x_e4m3 = per_token_cast_to_fp8(x)
    x_pure_rand_e4m3 = per_token_cast_to_fp8(x_pure_rand)
    x_e4m3 = (x_e4m3[0], x_e4m3[1].T.contiguous().T)
    scores = torch.randn((num_tokens, num_experts), dtype=torch.float32, device='xpu').abs() + 1
    group_scores = scores.view(num_tokens, num_nodes, -1).amax(dim=-1)
    group_idx = torch.topk(group_scores, k=num_topk_groups, dim=-1, sorted=False).indices
    masked_scores = create_grouped_scores(scores, group_idx, num_nodes)
    topk_idx = torch.topk(masked_scores, num_topk, dim=-1, largest=True, sorted=False)[1]
    topk_idx = topk_idx.to(deep_ep.topk_idx_t)
    topk_weights = torch.ones((num_tokens, num_topk), dtype=torch.float32, device='xpu') * rank
    topk_weights_pure_rand = torch.randn((num_tokens, num_topk), dtype=torch.float32, device='xpu')
    ...
    # Expert meta
    num_tokens_per_expert = torch.zeros((num_experts, ), dtype=torch.int, device='xpu')
    ...
    num_tokens_per_rdma_rank = torch.empty((num_nodes, ), dtype=torch.int, device='xpu')
    token_idx_in_rank = torch.full((num_ranks, num_tokens), -1, dtype=torch.long, device='xpu')
    for i in range(num_ranks):
        ...
        token_idx_in_rank[i][tokens[:count]] = torch.arange(count, dtype=torch.long, device='xpu')
        ...
        for async_mode in (False, True):
            for current_x in (x_pure_rand, x, x_pure_rand_e4m3, x_e4m3):
                for with_topk in (False, True):
                    ...
                    # Test combine
                    bias_0 = torch.ones((num_tokens, hidden), dtype=torch.bfloat16, device='xpu')
                    bias_1 = torch.randn((num_tokens, hidden), dtype=torch.bfloat16, device='xpu')
    ...
    for current_x in (x_e4m3, x):
        ...
        if isinstance(current_x, tuple):
            ...
            best_dispatch_results = torch.tensor([best_results[0], best_results[1], best_results[2]], dtype=torch.int32, device='xpu')
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    if args.test_ll_compatibility:
        test_low_latency.assert_low_latency_unsupported(buffer, group, rank)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test experimental XPU internode EP kernels')
