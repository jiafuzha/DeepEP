...
def require_xpu_devices(num_local_ranks: int, test_name: str) -> None:
    if num_local_ranks <= 0:
        raise RuntimeError(f'{test_name} requires at least one local process')
...
def bench_kineto(fn,
    ...
    with suppress():
        ...
        with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.XPU], schedule=schedule) as prof:
            for _ in range(2):
                # NOTES: use a large kernel and a barrier to eliminate the unbalanced CPU launch overhead
                if barrier_comm_profiling:
                    lhs = torch.randn((8192, 8192), dtype=torch.float, device='xpu')
                    rhs = torch.randn((8192, 8192), dtype=torch.float, device='xpu')
                    lhs @ rhs
                    dist.all_reduce(torch.ones(1, dtype=torch.float, device='xpu'))
    ...
    prof_lines = prof.key_averages().table(sort_by='xpu_time_total', max_name_column_width=100).split('\n')
    ...
    if num_kernels_per_period > 1:
        trace_path = Path.cwd() / f'.bench_kineto_trace_rank{dist.get_rank() if dist.is_initialized() else 0}_pid{os.getpid()}.json'
        try:
