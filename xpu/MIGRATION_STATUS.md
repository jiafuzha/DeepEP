...
- Added two-process cross-process IPC regression coverage in `xpu/tests/test_xpu_import.py` to verify exchanged foreign handles can drive single-rank **cached** intranode dispatch/combine success on staged XPU.
- Added two-process cross-process IPC regression coverage in `xpu/tests/test_xpu_import.py` to verify exchanged foreign handles can drive single-rank internode dispatch/combine success on staged XPU.
- Added two-process cross-process IPC regression coverage in `xpu/tests/test_xpu_import.py` to verify exchanged foreign handles can drive single-rank **cached** internode dispatch/combine success on staged XPU.
- Added two-process cross-process IPC regression coverage in `xpu/tests/test_xpu_import.py` to verify exchanged foreign handles can drive single-rank low-latency dispatch/combine success on staged XPU.
- Hardened cross-process exchanged-handle single-rank success paths in `xpu/tests/test_xpu_import.py` by running intranode/internode/low-latency dispatch+combine checks across multiple rounds per process pair.
...
- Added a constrained cross-process multi-rank intranode dispatch path in `xpu/csrc/deep_ep.cpp` that succeeds for local-only routing by collapsing staged execution to an effective single-rank flow, while preserving fast-fail behavior for remote-routing multi-rank cases.
- Added two-process cross-process IPC regression coverage in `xpu/tests/test_xpu_import.py` to verify exchanged handles can drive local-only two-rank intranode dispatch success across processes.
- Extended staged XPU `Buffer::intranode_combine` in `xpu/csrc/deep_ep.cpp` to mirror the constrained local-only cross-process behavior by collapsing to an effective single-rank combine flow when channel-prefix metadata proves local-only routing.
- Extended staged XPU `Buffer::intranode_combine` to accept both full-rank and collapsed cached local-only prefix metadata (`rank_prefix_matrix`/`channel_prefix_matrix`/`send_head`) for two-process IPC flows, enabling cached dispatch metadata to feed combine directly.
- Added two-process cross-process IPC regression coverage in `xpu/tests/test_xpu_import.py` to verify exchanged handles can drive local-only two-rank intranode dispatch+combine success across processes.
- Added two-process cross-process IPC regression coverage in `xpu/tests/test_xpu_import.py` to verify exchanged handles can drive partial-local two-rank intranode dispatch+combine success across processes for uncached and cached flows.
- Added two-process cross-process IPC regression coverage in `xpu/tests/test_xpu_import.py` to verify exchanged handles can drive multi-round partial-local two-rank internode dispatch+combine success across processes for uncached and cached flows.
- Added two-process cross-process IPC regression coverage in `xpu/tests/test_xpu_import.py` to verify exchanged handles can drive multi-round partial-local two-rank low-latency dispatch+combine success across processes.
- Repaired native-staged XPU build break in `xpu/csrc/deep_ep.cpp` by restoring corrupted low-latency dispatch/combine setup, fixing malformed low-latency combine signature wiring, and restoring missing XPU helper declarations/includes.
- Added `pybind11/functional.h` in `xpu/csrc/deep_ep.cpp` so low-latency return-hook APIs (`std::function<void()>`) convert correctly to Python in native extension bindings.
- Fixed cross-process cached local-only intranode combine shape handling in `xpu/csrc/deep_ep.cpp` to accept collapsed rank-prefix metadata (including `1x1` cached prefix matrices) in staged two-process flows.
- Revalidated native-staged XPU build and runtime after repairs: editable install succeeds and full XPU regression suite passes (`89 passed`).
- Updated root package installation behavior for non-CUDA environments in `setup.py` with a clear CUDA-missing error and an explicit metadata-only escape hatch (`DEEPEP_SKIP_CUDA_BUILD=1`), and documented the workflow in `README.md`.
...
- [x] IPC-group rendezvous identity stabilized from all-gathered handles (including self-rank) for cross-process staged grouping
- [x] Buffer lifecycle management (idempotent destroy)
- [x] Error handling and live allocation tracking
- All 89 XPU tests pass, including expanded native intranode regressions (uncached/cached two-rank two-process dispatch+combine plus float64/float32/float16 two-rank two-process combine and cached float16), uncached intranode float32/float16 multi-round two-process stress coverage, staged low-latency maintenance coverage, staged low-latency BF16/FP8 dispatch+combine coverage (including logfmt combine and return-recv-hook receive phase), staged two-rank low-latency dispatch/combine coverage (including FP8/logfmt two-process variants), staged internode dispatch/combine coverage (single-rank and two-rank two-process, including float32/float16 plus cached float32/float16), cached multi-round two-process stress coverage (including cached float16 intranode/internode stress), low-latency FP8/logfmt multi-round two-process stress coverage plus partial-local two-rank multi-round stress coverage, cross-process IPC `sync()` coverage with fail-fast guards for true intranode/internode remote-routing multi-rank dispatch paths, cross-process exchanged-handle single-rank intranode dispatch/combine success coverage, cross-process exchanged-handle single-rank cached intranode dispatch/combine success coverage, cross-process exchanged-handle single-rank internode dispatch/combine success coverage, cross-process exchanged-handle single-rank cached internode dispatch/combine success coverage, cross-process exchanged-handle single-rank low-latency dispatch/combine success coverage, cross-process exchanged-handle local-only two-rank intranode dispatch+combine success coverage, cross-process exchanged-handle local-only two-rank cached intranode dispatch+combine success coverage, cross-process exchanged-handle partial-local two-rank intranode dispatch+combine success coverage for uncached and cached paths, cross-process exchanged-handle partial-local two-rank internode dispatch+combine success coverage for uncached and cached paths plus uncached/cached multi-round stress coverage, cross-process exchanged-handle local-only and partial-local two-rank low-latency dispatch+combine success coverage, fallback tests, and import/runtime coverage
...
- Implement SYCL kernels for compute-critical operations:
  ...
  2. intranode::dispatch/combine (NVLink MoE gather/scatter) 🚧 staged two-rank same-process dispatch (uncached/cached) and combine implemented; staged PCIe-IPC handle-token keyed rendezvous path enabled on XPU, with group identity now sourced from all-gathered IPC handles (including self-rank) to keep cross-process staged grouping stable; cross-process IPC `sync()` coverage added with fast-fail guard for remote-routing staged multi-rank intranode across processes plus constrained local-only and partial-local two-rank cross-process dispatch+combine success for uncached and cached flows (including direct combine consumption of collapsed cached metadata); full SYCL parallelization and broader multi-rank hardening pending
  3. internode::dispatch/combine (RDMA MoE operations) 🚧 staged single-rank and two-rank same-process paths implemented; two-process cross-process local-only and partial-local dispatch+combine success implemented for uncached and cached flows; true cross-process remote-routing multi-rank dispatch/combine still fail-fast
  4. low-latency dispatch/combine (low-latency paths) ✅ staged single-rank and two-rank coverage implemented for BF16/FP8 dispatch, logfmt combine, maintenance helpers, return-recv-hook receive-phase execution, constrained local-only two-rank cross-process dispatch+combine success, and constrained partial-local two-rank cross-process dispatch+combine success (two-process validation)
