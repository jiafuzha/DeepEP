        ...
        if (warp_id < num_channels) {
            ...
            for (int dst_rdma_rank = sm_id - 2; dst_rdma_rank < num_rdma_ranks; dst_rdma_rank += num_channels * 2 - 2) {
                ...
                for (int batch_end_idx = token_end_idx; batch_end_idx > token_start_idx; batch_end_idx -= num_tokens_per_batch) {
                    auto batch_start_idx = std::max(token_start_idx, batch_end_idx - num_tokens_per_batch);
...
__device__ int combine_token(bool is_token_in_rank,
                             ...
                             int hidden_int4,
                             int num_topk,
                             sycl::int4* combined_row,
                             float* combined_topk_weights,
                             const sycl::int4* bias_0_int4,
                             const sycl::int4* bias_1_int4,
    ...
    for (int i = 0; i < kNumRanks; ++i)
        /*
        DPCT1121:3: Make sure that the "is_token_in_rank" which is used in the SYCL group function/algorithm is initialized.
        */
        if (dpct::select_from_sub_group(sycl::ext::oneapi::this_work_item::get_sub_group(), is_token_in_rank, i)) {
    ...
    if constexpr (kUseTMA) {
        ...
        auto tma_load_buffer = [=](const int& i, const int& j) -> sycl::int4* {
            return reinterpret_cast<sycl::int4*>(smem_ptr + i * kNumTMABufferBytesPerStage + j * kNumTMALoadBytes);
        };
        auto tma_store_buffer = [=](const int& i) -> sycl::int4* {
            return reinterpret_cast<sycl::int4*>(smem_ptr + i * kNumTMABufferBytesPerStage + NUM_MAX_NVL_PEERS * kNumTMALoadBytes);
        };
        ...
        for (int i = lane_id; i < hidden_int4; i += 32) {
            ...
            sycl::int4 bias_0_value_int4, bias_1_value_int4;
...
#define COMBINE_LAUNCH_CASE(num_rdma_ranks)                                           \
    ...
    EP_HOST_ASSERT(type == dpct::library_data_t::real_bfloat16);
