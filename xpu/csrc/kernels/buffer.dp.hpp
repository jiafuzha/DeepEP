...
public:
    ...
    __forceinline__ Buffer() : ptr(nullptr), total_bytes(0) {}

    __forceinline__ Buffer(void*& gbl_ptr, int num_elems, int offset = 0) {
    ...
    __forceinline__ Buffer advance_also(void*& gbl_ptr) {
        gbl_ptr = static_cast<uint8_t*>(gbl_ptr) + total_bytes;
        return *this;
    }

    __forceinline__ dtype_t* buffer() { return reinterpret_cast<dtype_t*>(ptr); }

    __forceinline__ dtype_t& operator[](int idx) { return buffer()[idx]; }
};
...
public:
    ...
    __forceinline__ AsymBuffer(void*& gbl_ptr, int num_elems, int num_ranks, int sm_id = 0, int num_sms = 1, int offset = 0) {
    ...
    __forceinline__ AsymBuffer(void** gbl_ptrs, int num_elems, int num_ranks, int sm_id = 0, int num_sms = 1, int offset = 0) {
        EP_STATIC_ASSERT(kNumRanks > 1, "");
        num_bytes = num_elems * sizeof(dtype_t);
    ...
    __forceinline__ void advance(int shift) {
        #pragma unroll
        for (int i = 0; i < kNumRanks; ++i)
            ptrs[i] = ptrs[i] + shift * sizeof(dtype_t);
    }

    __forceinline__ AsymBuffer advance_also(void*& gbl_ptr) {
        gbl_ptr = static_cast<uint8_t*>(gbl_ptr) + total_bytes;
        return *this;
    }

    template <int kNumAlsoRanks>
    __forceinline__ AsymBuffer advance_also(void** gbl_ptrs) {
        for (int i = 0; i < kNumAlsoRanks; ++i)
            gbl_ptrs[i] = static_cast<uint8_t*>(gbl_ptrs[i]) + total_bytes;
        return *this;
    }

    __forceinline__ dtype_t* buffer(int idx = 0) {
    ...
    __forceinline__ dtype_t* buffer_by(int rank_idx, int idx = 0) {
        EP_STATIC_ASSERT(kNumRanks > 1, "`buffer` is only available for single rank case");
...
public:
    ...
    __forceinline__ SymBuffer(void*& gbl_ptr, int num_elems, int num_ranks, int sm_id = 0, int num_sms = 1) {
        num_bytes = num_elems * sizeof(dtype_t);
    ...
    __forceinline__ dtype_t* send_buffer(int idx = 0) {
    ...
    __forceinline__ dtype_t* recv_buffer(int idx = 0) {
