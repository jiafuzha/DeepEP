...
struct NoTopkDispatchArgs {
    int4* recv_x;
    int* recv_src_idx;
    int* recv_channel_offset;
    int* send_head;
    const int4* x;
    const uint8_t* is_token_in_rank;
    const int* channel_prefix_matrix;
    int num_tokens;
    int hidden_int4;
    uint8_t** buffer_ptrs;
...
class TunedDispatchKernel;

template <int kNumRanks>
class TunedDispatchNoTopkKernel;

template <int kNumRanks>
class TunedDispatchNoTopkSendKernel;

template <int kNumRanks>
class TunedDispatchNoTopkRecvKernel;
...
__global__ void __launch_bounds__(kNumThreads, 1) dispatch(DispatchArgs args, int* shared_channel_tail_idx) {
    auto recv_x = args.recv_x;
    auto recv_x_scales = args.recv_x_scales;
    auto recv_src_idx = args.recv_src_idx;
    auto recv_topk_idx = args.recv_topk_idx;
    auto recv_topk_weights = args.recv_topk_weights;
    auto recv_channel_offset = args.recv_channel_offset;
    auto send_head = args.send_head;
    auto x = args.x;
    auto x_scales = args.x_scales;
    auto topk_idx = args.topk_idx;
    auto topk_weights = args.topk_weights;
    auto is_token_in_rank = args.is_token_in_rank;
    auto channel_prefix_matrix = args.channel_prefix_matrix;
    auto num_tokens = args.num_tokens;
    auto num_worst_tokens = args.num_worst_tokens;
    auto hidden_int4 = args.hidden_int4;
    auto num_topk = args.num_topk;
    auto num_experts = args.num_experts;
    auto num_scales = args.num_scales;
    auto scale_token_stride = args.scale_token_stride;
    auto scale_hidden_stride = args.scale_hidden_stride;
    auto buffer_ptrs = args.buffer_ptrs;
    auto rank = args.rank;
    auto num_max_send_tokens = args.num_max_send_tokens;
    auto num_recv_buffer_tokens = args.num_recv_buffer_tokens;
    ...
    auto* layout_ptr = static_cast<uint8_t*>(buffer_ptrs[is_sender ? responsible_rank : rank]) +
    ...
    auto* channel_start_offset = reinterpret_cast<int*>(layout_ptr) + channel_rank_offset;
    layout_ptr += num_channels_total * sizeof(int);
    auto* channel_end_offset = reinterpret_cast<int*>(layout_ptr) + channel_rank_offset;
    layout_ptr += num_channels_total * sizeof(int);
    auto* channel_head_idx = reinterpret_cast<int*>(layout_ptr) + channel_rank_offset;
    layout_ptr += num_channels_total * sizeof(int);
    auto* channel_tail_idx = reinterpret_cast<int*>(layout_ptr) + channel_rank_offset;
    layout_ptr += num_channels_total * sizeof(int);
    ...
    auto* channel_x_buffers = reinterpret_cast<int4*>(layout_ptr) + channel_rank_offset * num_recv_buffer_tokens * hidden_int4;
    layout_ptr += static_cast<size_t>(num_channels_total) * num_recv_buffer_tokens * hidden_int4 * sizeof(int4);
    auto* channel_src_idx_buffers = reinterpret_cast<int*>(layout_ptr) + channel_rank_offset * num_recv_buffer_tokens;
    layout_ptr += static_cast<size_t>(num_channels_total) * num_recv_buffer_tokens * sizeof(int);
    auto* channel_topk_idx_buffers =
        reinterpret_cast<topk_idx_t*>(layout_ptr) + channel_rank_offset * num_recv_buffer_tokens * num_topk;
    layout_ptr += static_cast<size_t>(num_channels_total) * num_recv_buffer_tokens * num_topk * sizeof(topk_idx_t);
    auto* channel_topk_weights_buffers =
        reinterpret_cast<float*>(layout_ptr) + channel_rank_offset * num_recv_buffer_tokens * num_topk;
    layout_ptr += static_cast<size_t>(num_channels_total) * num_recv_buffer_tokens * num_topk * sizeof(float);
    auto* channel_x_scales_buffers =
        reinterpret_cast<float*>(layout_ptr) + channel_rank_offset * num_recv_buffer_tokens * num_scales;
    ...
    if (is_sender) {
        ...
        if (send_warp_id_in_rank == 0 and elect_one_sync()) {
            ...
            st_relaxed_sys_global(channel_start_offset, -value - 1);
            value = channel_prefix_matrix[responsible_rank * num_channels + responsible_channel];
            st_relaxed_sys_global(channel_end_offset, -value - 1);
        }
        sycl::group_barrier(item_ct1.get_sub_group());
        ...
        for (int64_t token_idx = token_start_idx; token_idx < token_end_idx;) {
            ...
            if (elect_one_sync()) {
                while (true) {
                    ...
                    int num_used_slots = cached_channel_tail_idx - ld_volatile_global(channel_head_idx);
                    if (num_recv_buffer_tokens - num_used_slots >= num_max_send_tokens)
                        break;

                    // Rare cases to loop again
                    if (++wait_iters > NUM_TIMEOUT_CYCLES) {
                        trap();
                    }
                }
            }
            ...
            while (chunk_token_idx < num_max_send_tokens and token_idx < token_end_idx) {
                ...
                if (cached_channel_tail_idx % num_send_warps_per_rank == send_warp_id_in_rank) {
                    // Copy data
                    auto shifted_channel_x_buffers = channel_x_buffers + dst_slot_idx * hidden_int4;
            ...
            sycl::group_barrier(item_ct1.get_group());
            if (send_warp_id_in_rank == 0 and elect_one_sync())
                st_release_sys_global(channel_tail_idx, cached_channel_tail_idx);
        }
    } else {
        ...
        if (elect_one_sync()) {
            while ((total_offset = ld_volatile_global(channel_start_offset)) == 0)
                ;
            while ((num_tokens_to_recv = ld_volatile_global(channel_end_offset)) == 0)
                ;
        ...
        total_offset = sycl::group_broadcast(sg, total_offset, sycl::sub_group::linear_id_type{0});
        total_offset += rank_offset;
        num_tokens_to_recv = sycl::group_broadcast(sg, num_tokens_to_recv, sycl::sub_group::linear_id_type{0});
        ...
        while (num_tokens_to_recv > 0) {
            ...
            while (recv_thread_id_in_rank == 0) {
                cached_channel_tail_idx = ld_acquire_sys_global(channel_tail_idx);
            ...
            for (int chunk_idx = recv_warp_id_in_rank; chunk_idx < num_recv_tokens; chunk_idx += num_recv_warps_per_rank) {
                ...
                auto shifted_buffer_x_int4 = channel_x_buffers + token_idx_in_buffer * hidden_int4;
            ...
            for (int chunk_idx = cached_channel_head_idx + recv_thread_id_in_rank; chunk_idx < cached_channel_tail_idx;
                ...
                recv_src_idx[total_offset + chunk_idx - cached_channel_head_idx] =
                    ld_nc_global(channel_src_idx_buffers + chunk_idx % num_recv_buffer_tokens);
            ...
            for (int idx = recv_thread_id_in_rank; idx < num_recv_tokens * num_topk; idx += 32 * num_recv_warps_per_rank) {
                ...
                recv_topk_idx[recv_idx] = ld_nc_global(channel_topk_idx_buffers + buffer_idx);
                recv_topk_weights[recv_idx] = ld_nc_global(channel_topk_weights_buffers + buffer_idx);
            }
            ...
            for (int i = recv_thread_id_in_rank; i < num_recv_tokens * num_scales; i += 32 * num_recv_warps_per_rank) {
                ...
                recv_x_scales[static_cast<int64_t>(total_offset + chunk_idx) * num_scales + scales_idx] =
                    ld_nc_global(channel_x_scales_buffers + token_idx_in_buffer * num_scales + scales_idx);
            }
            ...
            if (recv_warp_id_in_rank == num_recv_warps_per_rank - 1 and elect_one_sync())
                st_relaxed_sys_global(channel_head_idx, cached_channel_head_idx);
...
__global__ void __launch_bounds__(kNumThreads, 1) dispatch_no_topk(NoTopkDispatchArgs args, int* shared_channel_tail_idx) {
    auto recv_x = args.recv_x;
    auto recv_src_idx = args.recv_src_idx;
    auto recv_channel_offset = args.recv_channel_offset;
    auto send_head = args.send_head;
...
template <int kNumRanks, int kNumThreads>
__global__ void __launch_bounds__(kNumThreads, 1) dispatch_no_topk_send(int* send_head,
                                                                        const int4* x,
                                                                        const uint8_t* is_token_in_rank,
                                                                        const int* channel_prefix_matrix,
                                                                        int num_tokens,
                                                                        int hidden_int4,
                                                                        void* const* buffer_ptrs,
                                                                        int rank,
                                                                        int num_max_send_tokens,
                                                                        int num_recv_buffer_tokens) {
    const auto thread_id = static_cast<int>(threadIdx.x), lane_id = get_lane_id();
    const auto responsible_channel = static_cast<int>(blockIdx.x);
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    ...
    auto num_channels = static_cast<int>(gridDim.x);
    auto num_channels_total = num_channels * kNumRanks;
    auto channel_rank_offset = responsible_channel * kNumRanks + rank;
    auto* layout_ptr = static_cast<uint8_t*>(buffer_ptrs[responsible_rank]) + kNumRanks * kNumRanks * sizeof(int);
    auto* channel_start_offset = reinterpret_cast<int*>(layout_ptr) + channel_rank_offset;
    layout_ptr += num_channels_total * sizeof(int);
    auto* channel_end_offset = reinterpret_cast<int*>(layout_ptr) + channel_rank_offset;
    layout_ptr += num_channels_total * sizeof(int);
    ...
    for (int64_t token_idx = token_start_idx; token_idx < token_end_idx;) {
        ...
        while (chunk_token_idx < num_max_send_tokens and token_idx < token_end_idx) {
            if (token_idx % num_send_warps_per_rank == send_warp_id_in_rank and elect_one_sync())
                send_head[token_idx * kNumRanks + responsible_rank] =
                    is_token_in_rank[token_idx * kNumRanks + responsible_rank] != 0 ? cached_channel_tail_idx : -1;

            if (is_token_in_rank[token_idx * kNumRanks + responsible_rank] == 0) {
                token_idx++;
                continue;
            }
...
template <int kNumRanks, int kNumThreads>
__global__ void __launch_bounds__(kNumThreads, 1) dispatch_no_topk_recv(NoTopkDispatchArgs args) {
    auto recv_x = args.recv_x;
    auto recv_src_idx = args.recv_src_idx;
    auto recv_channel_offset = args.recv_channel_offset;
    auto hidden_int4 = args.hidden_int4;
    ...
    auto num_channels = static_cast<int>(gridDim.x);
    auto num_channels_total = num_channels * kNumRanks;
    auto channel_rank_offset = responsible_channel * kNumRanks + responsible_rank;
    auto* layout_ptr = static_cast<uint8_t*>(buffer_ptrs[rank]) + kNumRanks * kNumRanks * sizeof(int);
    auto* channel_start_offset = reinterpret_cast<int*>(layout_ptr) + channel_rank_offset;
    layout_ptr += num_channels_total * sizeof(int);
    auto* channel_end_offset = reinterpret_cast<int*>(layout_ptr) + channel_rank_offset;
    layout_ptr += num_channels_total * sizeof(int);
    ...
    int total_offset = 0, num_tokens_to_recv = 0, cached_channel_head_idx = 0, cached_channel_tail_idx = 0;
    if (elect_one_sync()) {
        total_offset = -ld_volatile_global(channel_start_offset) - 1;
        num_tokens_to_recv = -ld_volatile_global(channel_end_offset) - 1 - total_offset;
        cached_channel_head_idx = ld_volatile_global(channel_head_idx);
        cached_channel_tail_idx = ld_acquire_sys_global(channel_tail_idx);
        if (recv_warp_id_in_rank == 0)
            recv_channel_offset[responsible_rank * num_channels + responsible_channel] = total_offset;
    }
    total_offset = sycl::group_broadcast(sg, total_offset, sycl::sub_group::linear_id_type{0}) + rank_offset;
    num_tokens_to_recv = sycl::group_broadcast(sg, num_tokens_to_recv, sycl::sub_group::linear_id_type{0});
    cached_channel_head_idx = sycl::group_broadcast(sg, cached_channel_head_idx, sycl::sub_group::linear_id_type{0});
    cached_channel_tail_idx = sycl::group_broadcast(sg, cached_channel_tail_idx, sycl::sub_group::linear_id_type{0});

    int num_recv_tokens = cached_channel_tail_idx - cached_channel_head_idx;
    EP_DEVICE_ASSERT(num_recv_tokens == num_tokens_to_recv);
    ...
    sycl::group_barrier(item_ct1.get_group());
    if (recv_warp_id_in_rank == num_recv_warps_per_rank - 1 and elect_one_sync())
        st_relaxed_sys_global(channel_head_idx, cached_channel_tail_idx);
}
...
void dispatch(void* recv_x,
    ...
    const bool use_no_topk_dispatch = (num_topk == 0 and num_scales == 0);
    NoTopkDispatchArgs no_topk_args{reinterpret_cast<int4*>(recv_x),
                                    recv_src_idx,
                                    recv_channel_offset,
                                    send_head,
                                    reinterpret_cast<const int4*>(x),
                                    reinterpret_cast<const uint8_t*>(is_token_in_rank),
                                    channel_prefix_matrix,
                                    num_tokens,
                                    hidden_int4,
                                    reinterpret_cast<uint8_t**>(buffer_ptrs),
                                    rank,
                                    num_max_send_tokens,
                                    num_recv_buffer_tokens};
    auto launch_with_local = [&](auto ranks_tag) {
        constexpr int ranks = decltype(ranks_tag)::value;
        if (use_no_topk_dispatch) {
            const auto nd_range =
                sycl::nd_range<3>(sycl::range<3>(1, 1, (num_sms / 2) * kNumThreads), sycl::range<3>(1, 1, kNumThreads));
            auto* x_int4 = reinterpret_cast<const int4*>(x);
            auto* is_token_in_rank_u8 = reinterpret_cast<const uint8_t*>(is_token_in_rank);
            auto* raw_buffer_ptrs = static_cast<void* const*>(buffer_ptrs);
            stream->submit([&](sycl::handler& cgh) {
                cgh.parallel_for<TunedDispatchNoTopkSendKernel<ranks>>(nd_range, [=](sycl::nd_item<3>) [[sycl::reqd_sub_group_size(32)]] {
                    dispatch_no_topk_send<ranks, kNumThreads>(send_head,
                                                              x_int4,
                                                              is_token_in_rank_u8,
                                                              channel_prefix_matrix,
                                                              num_tokens,
            ...
            stream->submit([&](sycl::handler& cgh) {
                cgh.parallel_for<TunedDispatchNoTopkRecvKernel<ranks>>(nd_range, [=](sycl::nd_item<3>) [[sycl::reqd_sub_group_size(32)]] {
                    dispatch_no_topk_recv<ranks, kNumThreads>(no_topk_args);
                });
            });
        } else {
            stream->submit([&](sycl::handler& cgh) {
                sycl::local_accessor<int, 1> shared_channel_tail_idx(sycl::range<1>(ranks), cgh);
                cgh.parallel_for<TunedDispatchKernel<ranks>>(
                    sycl::nd_range<3>(sycl::range<3>(1, 1, num_sms * kNumThreads), sycl::range<3>(1, 1, kNumThreads)),
                    [=](sycl::nd_item<3>) [[sycl::reqd_sub_group_size(32)]] {
                        DispatchArgs args{reinterpret_cast<int4*>(recv_x), recv_x_scales,          recv_src_idx,       recv_topk_idx, recv_topk_weights,
                                          recv_channel_offset,          send_head,                  reinterpret_cast<const int4*>(x),
                                          x_scales,                     topk_idx,                   topk_weights,        is_token_in_rank,
                                          channel_prefix_matrix,        num_tokens,                 num_worst_tokens,    hidden_int4,
                                          num_topk,                     num_experts,                num_scales,          scale_token_stride,
                                          scale_hidden_stride,          buffer_ptrs,                rank,                num_max_send_tokens,
                                          num_recv_buffer_tokens};
                        auto shared_channel_tail_idx_ptr =
                            shared_channel_tail_idx.template get_multi_ptr<sycl::access::decorated::no>().get();
                        dispatch<ranks, kNumThreads, kNumTMABytesPerWarp>(args, shared_channel_tail_idx_ptr);
                    });
            });
        }
    };
    switch (num_ranks) {
        case 2:
            launch_with_local(std::integral_constant<int, 2>{});
            break;
        case 4:
            launch_with_local(std::integral_constant<int, 4>{});
            break;
        case 8:
            launch_with_local(std::integral_constant<int, 8>{});
            break;
        default:
            EP_HOST_ASSERT(false and "Unsupported ranks");
    }
...
__global__ void cached_notify_combine(
    ...
    if (sm_id == 0) {
        ...
        for (int token_idx_tail = token_end_idx - 1; token_idx_tail >= token_start_idx; token_idx_tail -= 32) {
            ...
            for (int i = 0; i < std::min(32, token_idx_tail - token_start_idx + 1); ++i) {
...
void cached_notify_combine(void** buffer_ptrs,
    ...
    auto launch = [&](auto ranks_tag) {
        constexpr int ranks = decltype(ranks_tag)::value;
        stream->submit([&](sycl::handler& cgh) {
            cgh.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, (1 + num_channels) * num_threads), sycl::range<3>(1, 1, num_threads)),
                             [=](sycl::nd_item<3>) {
...
__global__ void __launch_bounds__(kNumThreads, 1) combine(dtype_t* recv_x,
                                                          ...
                                                          int num_max_send_tokens,
                                                          int num_recv_buffer_tokens,
                                                          int* warp_channel_head_idx,
                                                          int* channel_tail_idx,
                                                          int* warp_retired) {
        ...
        if (thread_id < 32) {
            ...
            while (lane_id < kNumRanks) {
                ...
                for (int i = 1; i < num_recv_warps; ++i)
                    retired = retired and (warp_retired[i] != 0);
                if (retired)
                    break;
                ...
                for (int i = 1; i < num_recv_warps; ++i)
                    if (warp_retired[i] == 0)
                        min_head = min(min_head, warp_channel_head_idx[i * kNumRanks + lane_id]);
            ...
            for (int64_t token_idx = token_start_idx + recv_warp_id - 1; token_idx < token_end_idx; token_idx += num_recv_warps - 1) {
                ...
                // Update head
                if (lane_id < kNumRanks)
                    warp_channel_head_idx[recv_warp_id * kNumRanks + lane_id] = (expected_head < 0) ? -expected_head - 1 : expected_head + 1;
            }

            // Retired
            sycl::group_barrier(item_ct1.get_sub_group());
            if (elect_one_sync())
                warp_retired[recv_warp_id] = 1;
        }
    }
}

void combine(dpct::library_data_t type,
             void* recv_x,
             float* recv_topk_weights,
             const void* x,
             const float* topk_weights,
    ...
    switch (type) {
        case dpct::library_data_t::real_bfloat16: {
            auto launch = [&](auto ranks_tag) {
                ...
                stream->submit([&](sycl::handler& cgh) {
                    constexpr int num_recv_warps = kNumThreads / 32;
                    sycl::local_accessor<int, 2> warp_channel_head_idx(sycl::range<2>(num_recv_warps, ranks), cgh);
                    sycl::local_accessor<int, 1> channel_tail_idx(sycl::range<1>(ranks), cgh);
                    sycl::local_accessor<int, 1> warp_retired(sycl::range<1>(num_recv_warps), cgh);
                    cgh.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, num_sms * kNumThreads), sycl::range<3>(1, 1, kNumThreads)),
                                     [=](sycl::nd_item<3>) [[sycl::reqd_sub_group_size(32)]] {
                                         auto warp_channel_head_idx_ptr =
                                             warp_channel_head_idx.template get_multi_ptr<sycl::access::decorated::no>().get();
                                         auto channel_tail_idx_ptr =
                                             channel_tail_idx.template get_multi_ptr<sycl::access::decorated::no>().get();
                                         auto warp_retired_ptr = warp_retired.template get_multi_ptr<sycl::access::decorated::no>().get();
                                         combine<nv_bfloat16, ranks, kNumThreads, kNumTMABytesPerWarp>(
