#!/usr/bin/env bash
# ===========================================================================
# Launch 2-node DeepEP internode LOW-LATENCY test  -- v3
#
# Recreated on the isolated `deepep_jiafuzha_oneapi2026` image foundation (docker/). No host
...
#   deepep-ll-v3-node0: 127.0.0.1:2330, GPUs 0,1, NICs mlx5_0,mlx5_1
#   deepep-ll-v3-node1: 127.0.0.1:2331, GPUs 2,3, NICs mlx5_2,mlx5_3
#
# mpirun is launched INSIDE deepep-ll-v3-node0 (via docker exec) and spawns ranks
# on deepep-ll-v3-node1 via SSH. With ppn=2 the topology is:
...
up() {
    ...
    if [ "${DEEP_EP_FORCE_BUILD:-0}" = "1" ] || ! docker image inspect "${IMAGE_NAME:-deepep_jiafuzha_oneapi2026}" >/dev/null 2>&1; then
...
run_test() {
    ...
    local WRAPPER_PATH="$DEEP_EP_DIR/tests/docker-2node-ll-v3/node_wrapper.sh"
    ...
    echo "===== Setting up IGC FunctionDebug.txt (IntelIGC output folder) ====="
    # In oneAPI 2026.1, IGC_SelectiveFunctionControl=1 reads FunctionDebug.txt from
    # the IGC output folder (/tmp/IntelIGC/), NOT from CWD. The file must contain
    # ALL transitively-called non-inlined iSHMEM functions (not just the top-level
    # API functions), as 2026.1 IGC no longer follows the call chain from force-inlined
    # callees. See FunctionDebug_ishmem_inline_2026_1.txt and the updated doc.
    local FD_SRC="$DEEP_EP_DIR/tests/docker-2node-ll-v3/FunctionDebug_ishmem_inline_2026_1.txt"
    docker exec "$NODE0_CONTAINER" bash -lc "mkdir -p /tmp/IntelIGC && cp $FD_SRC /tmp/IntelIGC/FunctionDebug.txt" 2>/dev/null || true
    docker exec "$NODE1_CONTAINER" bash -lc "mkdir -p /tmp/IntelIGC && cp $FD_SRC /tmp/IntelIGC/FunctionDebug.txt" 2>/dev/null || true
    ...
    docker exec \
        ...
        -e IGC_SelectiveFunctionControl=1 \
        -e IGC_FunctionControl=0 \
        ...
        -e ISHMEM_IBGDA_QUIET_SKIP_DRAIN="${ISHMEM_IBGDA_QUIET_SKIP_DRAIN:-1}" \
        ...
        bash -lc "
            ...
            timeout $TIMEOUT_SEC mpirun \
                ...
                -genv IGC_SelectiveFunctionControl 1 \
                -genv IGC_FunctionControl 0 \
