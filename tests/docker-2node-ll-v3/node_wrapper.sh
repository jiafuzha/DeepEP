#!/bin/bash
# Per-rank wrapper executed inside each MPI rank (docker-2node-ll-v3, LOW-LATENCY).
#
...
# ZE_ENABLE_PCI_ID_DEVICE_ORDER must be set BEFORE setvars/oneAPI init.
export ZE_ENABLE_PCI_ID_DEVICE_ORDER=1
# Fallback: derive ZE_AFFINITY_MASK from hostname if /run/deepep_node_env failed.
# node0 → 0,1; node1 → 2,3 (default smc26 topology).
if [ -z "${ZE_AFFINITY_MASK:-}" ]; then
    case "$(hostname)" in
        *node1) export ZE_AFFINITY_MASK=2,3 ;;
...
# Ensure all 2026.1 runtime libs are accessible before 2025.3 fallbacks.
# setvars already puts MKL/MPI/TBB in the path; ensure compiler lib comes first.
export LD_LIBRARY_PATH=/opt/intel/oneapi/compiler/2026.1/lib:${LD_LIBRARY_PATH}

# Patched IGC (isFuncGroupHead guard for NBarrierCnt fix) — must come BEFORE
# the system libigc.so.2 so the NEO GPU driver picks up the fix at JIT time.
if [ -f /opt/igc-patched/lib/libigc.so ]; then
    export LD_LIBRARY_PATH=/opt/igc-patched/lib:${LD_LIBRARY_PATH}
fi

# SYCL compat stub for handler:: symbols missing from libsycl.so.9
_STUB=/root/jiafuzha/code-repo/zjf2012/DeepEP/tests/docker-2node-ll-v3/handler_stub.so
if [ -f "$_STUB" ]; then
...
# IGC FunctionDebug.txt (must be in the IGC output folder /tmp/IntelIGC/ in
# oneAPI 2026.1, NOT in CWD). The mpirun -genv sets IGC_SelectiveFunctionControl=1
# and IGC_FunctionControl=0; this file enables selective inline of iSHMEM
# transitively-called functions to avoid the NBarrierCnt vISA conflict.
FD_SRC=/root/jiafuzha/code-repo/zjf2012/DeepEP/tests/docker-2node-ll-v3/FunctionDebug_ishmem_inline_2026_1.txt
if [ -f "$FD_SRC" ]; then
    mkdir -p /tmp/IntelIGC && cp "$FD_SRC" /tmp/IntelIGC/FunctionDebug.txt
fi
unset FD_SRC

# DeepEP / iSHMEM needs MASTER_ADDR (rank-0 node hostname).
export MASTER_ADDR=${MASTER_ADDR:-deepep-ll-v3-node0}
...
_DBGDIR=/root/jiafuzha/code-repo/zjf2012/DeepEP/tests/docker-2node-ll-v3
